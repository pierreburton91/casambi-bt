...
from ._switch import parseSwitchEvents
...
class CasambiClient:
    ...
    async def _writeEncPacket(self, packet: bytes, id: int, char_uuid: str) -> None:
        """Write an encrypted packet to a characteristic."""
        encPacket = self._encryptor.encryptThenMac(packet, self._getNonce(id))
        if self._connection is None:
            raise ConnectionStateError(
                ConnectionState.AUTHENTICATED, self._connectionState
            )
        try:
